...
export const GET: RequestHandler = async () => {
	try {
		...
		if (hist.length == 0) {
			...
			if (groups.length > 0) {
				const insertValues = groups.map((group) => [group.when, group.dists, group.frags, group.sinceLastPump, group.time]);
			...
			if (newGroups.length > 0) {
				...
				// Filter out any groups that have the same timestamp as existing records
				const existingTimes = new Set(hist.map((h) => h.when.getTime()));
				const uniqueNewGroups = newGroups.filter((group) => !existingTimes.has(new Date(group.when).getTime()));

				if (uniqueNewGroups.length > 0) {
					const insertValues = uniqueNewGroups.map((group) => [group.when, group.dists, group.frags, group.sinceLastPump, group.time]);
					...
					try {
						...
						console.log(`Inserted ${uniqueNewGroups.length} new records`);
						return json({ message: 'ok', fillSessions: uniqueNewGroups.concat(hist) });
					} catch (error) {
						console.error('Insert error:', error);
						// If insert fails, still return the combined data
						return json({ message: 'ok', fillSessions: uniqueNewGroups.concat(hist) });
					}
				}
			}
		...
		return json({ message: 'error', error: error instanceof Error ? error.message : String(error) }, { status: 500 });
	}
};
